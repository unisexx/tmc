...
<div class="row g-3">
    ...
    {{-- ====================== การตรวจสอบและอนุมัติ (Admin Only) ====================== --}}
    {{-- @can('approve-users') --}}
    <hr class="my-4">
    <h5>6. การตรวจสอบและอนุมัติ (สำหรับผู้ดูแลระบบ)</h5>
        ...
        <div class="row g-3">
            ...
            <div class="col-md-4">
                ...
                <select name="reg_status" id="reg_status" class="form-select">
                    @php
                        $status = old('reg_status', $user->reg_status ?? 'pending');
                    @endphp
                    <option value="pending" @selected($status === 'pending')>รอตรวจสอบ (pending)</option>
                    <option value="approved" @selected($status === 'approved')>อนุมัติ (approved)</option>
                    <option value="rejected" @selected($status === 'rejected')>ไม่อนุมัติ (rejected)</option>
                </select>
                @error('reg_status')
                    <div class="text-danger small mt-1">{{ $message }}</div>
                @enderror
